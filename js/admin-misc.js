...
function loadNavOrder(callback){
  apiFetch('admin.php','POST',{action:'get_setting',key:'nav_order'}).then(function(d){
    ...
    // Prune secs no longer known (e.g. removed features left over in saved nav_order)
    structure=structure.filter(function(n){return n.type==='folder'||ADMIN_NAV_LABELS[n.sec];}).map(function(n){
    ...
    // One-time migration: add PayPal Payments into Shop right after Square Payments, on saved
    // nav_orders that predate it (a fresh install picks this up from the folder defaults above).
    (function(){
      var inShop=structure.some(function(n){return n.type==='folder'&&n.sec==='shop'&&(n.children||[]).indexOf('paypalpay')>=0;});
      if(inShop)return;
      structure.forEach(function(n){
        if(n.type==='folder'&&n.sec==='shop'){
          var idx=n.children.indexOf('sqpay');
...
function _renderGitLog(el,d,deploys){
  ...
  var rows=d.commits.map(function(c){
    ...
    return '<tr><td style="white-space:nowrap">'+dateStr+'<br><span style="font-size:.78rem;color:#999">'+timeStr+'</span></td>'+
      '<td style="white-space:pre-wrap">'+escHtml(c.message)+'</td>'+
