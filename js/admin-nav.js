...
function aNavById(sec){
  ...
  var titles={dash:'Dashboard',prods:'Product Management',orders:'Orders',custs:'Customers',inv:'Inventory',sales:'Sales Report',subs:'Newsletter Subscribers',blast:'Email Blast',faqs:'FAQs',reviews:'Reviews',cats:'Categories',shipping:'Shipping Charges',sqpay:'Square Payments',sweep:'Tax Sweep',regtest:'Regression Tests',bizprofile:'Profile',bizdocs:'Documents',bizinv:'Business Inventory',bizreports:'Business Reports',bizequip:'Capital Equipment',emaillog:'Email Log',logs:'Error Logs',settings:'Settings',tncity:'TN City Sales Taxes',gitlog:'Change History',deploylog:'Deploy History',dbbackup:'DB Backup',logout:'Logout'};
  ...
  else if(sec==='custs')rCusts(el);else if(sec==='inv')rInv(el);else if(sec==='sales')rSales(el);else if(sec==='subs')rSubs(el);else if(sec==='blast')rBlast(el);else if(sec==='tncity')rTnCity(el);else if(sec==='faqs')rAdminFAQs(el);else if(sec==='reviews')rAdminReviews(el);else if(sec==='cats')rCats(el);else if(sec==='shipping')rShipping(el);else if(sec==='emaillog')rEmailLog(el);else if(sec==='logs')rLogs(el);else if(sec==='settings')rSettings(el);else if(sec==='bizprofile')rBizProfile(el);else if(sec==='bizdocs')rBizDocs(el);else if(sec==='bizinv')rBizInv(el);else if(sec==='bizreports')rBizReports(el);else if(sec==='bizequip')rBizEquip(el);else if(sec==='sweep')rSweep(el);else if(sec==='regtest')rRegTest(el);else if(sec==='sqpay')rSqPay(el);else if(sec==='gitlog')rGitLog(el);else if(sec==='deploylog')rDeployLog(el);else if(sec==='dbbackup')rDbBackup(el);
}
